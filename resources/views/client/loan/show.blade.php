            ...
            <div>
                ...
                <div class="row">
                    ...
                    <div class="col-3">
                        ...
                        <p>{{ auth()->user()->representative->name }}&nbsp;{{ auth()->user()->representative->surname }}
                        </p>
                    </div>
                    <div class="col-3">
                        <label class="mb-2" for="">Cedula : </label>
                        <p>{{ auth()->user()->representative->dni }}</p>
                    </div>
                    <div class="col-3">
                        <label class="mb-2" for=""> correo: </label>
                        <p>{{ auth()->user()->representative->email->email }}</p>
                    </div>
                    <div class="col-3">
                        <label class="mb-2" for="">Telefono : </label>
                        <p>{{ auth()->user()->representative->phone->phone }}</p>
                    </div>
                    <div class="col-3">
                        <label class="mb-2" for="">Direccion Completa: </label>

                        <p>
                            {{ auth()->user()->representative->address->address }},
                            {{ auth()->user()->representative->address->city
                                ? auth()->user()->representative->address->city->city .
                                    ',' .
                                    auth()->user()->representative->address->city->state->state
                                : auth()->user()->representative->address->parish->parish .
                                    ', ' .
                                    auth()->user()->representative->address->parish->township->township .
                                    ', ' .
                                    auth()->user()->representative->address->parish->township->state->state }}.
                        </p>
                    </div>
                    @foreach (auth()->user()->representative->socialMedia as $socialMedia)
                        <div class="col-3">
                            <label for="" class="mb-2">{{ $socialMedia->name }}:</label>
                ...
                @foreach ($loan->user->personalReferences as $personalReference)
                    <div class="row">
                        <div class="col-3">
                            <label class="mb-2" for="">Nombre y Apellido: </label>
                            <p>{{ $personalReference->name }}&nbsp;{{ $personalReference->surname }}
                            </p>
                        </div>
                        <div class="col-3">
                            <label class="mb-2" for="">Cedula : </label>
                ...
                @if ($loan->fees()->count() > 0)
                    ...
                    @foreach ($loan->fees as $fee)
                        <div class="row">
                            ...
                            <div class="col-3">
                                <label class="mb-2" for=""> Monto Abonado: </label>
                                <p>{{ $fee->gross_amount }}</p>
                            </div>
                            <div class="col-3">
                                <label class="mb-2" for="">Interés(3%) :</label>
                                <p>{{ $fee->interest_amount }}</p>
                            </div>
                            <div class="col-3">
                                <label class="mb-2" for="">Monto pagado: </label>
                                <p>{{ $fee->net_amount }}</p>
                            </div>
                            <div class="col-3">
                                <label class="mb-2" for="">Método de pago: </label>
                ...
                <div class="row">
                    <div class="col d-flex align-items-center justify-content-center">
                        ...
                        <a href="{{ route('client.loan.index') }}" class="btn btn-dark w-100">Atras</a>

                        @switch($loan->statement->status)
                            @case('disbursed')
                                @role('client|root|admin')
                                    <a href="{{ route('fee.create', $loan) }}" type="button"
                                        class="btn btn-success w-100">Abonar</a>
                                @endrole
                            @break
